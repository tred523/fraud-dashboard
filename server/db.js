...
function parseEvent(raw) {
  ...
  const event = {
    ...
    first_seen_at:          toInt(raw.first_seen_at),
    last_seen_at:           toInt(raw.last_seen_at),
    ip_address:             raw.ip_address || raw.ip || null,
    browser_name:           toStr(raw.browser_name || raw.browserName),
    os:                     toStr(raw.os),
    device:                 toStr(raw.device),
    bot:                    botVal,
    ...
    velocity_distinct_ip_24h: toInt(raw.velocity_distinct_ip_24h) ?? 0,
    velocity_events_24h:    toInt(raw.velocity_events_24h) ?? 0,
    ...
    confidence_score:       toFloat(raw.confidence_score),
  };
