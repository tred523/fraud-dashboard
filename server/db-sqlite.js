...
function initDb() {
  ...
  const demoKey = raw.prepare("SELECT id FROM api_keys WHERE key = 'demo_key_12345'").get();
  if (!demoKey) {
    raw.prepare("INSERT INTO api_keys (key, client_name, created_at) VALUES ('demo_key_12345', 'Demo Client', ?)").run(Date.now());
  }
